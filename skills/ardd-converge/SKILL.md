...
2. **Resolve mode and delegation gate.** Read `workflow_mode` from
   ...
   **Consult the `delegation` knob.** Read `delegation` from
   ...
   - `eager` — delegate to a background worktree subagent **without
     ...
     offer.)
   - `ask` (or absent) — **offer delegation, suggesting "yes."** Ask:
     - "Yes, delegate to a background subagent in an isolated worktree"
       (recommended)
     - "No, continue inline on the current branch"
   - `inline` — proceed inline at step 3 without offering.

   On **no** (or `delegation: inline`), continue inline at step 3 on the
   current branch (if
