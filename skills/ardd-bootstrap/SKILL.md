...
| `/ardd-plan` | Generate an implementation plan from all stable artifacts |
| `/ardd-tasks` | Generate an ordered task list from an approved plan |
| `/ardd-render [artifact]` | Generate Mermaid diagrams into `README.md`. Bare form runs all supported artifacts. |
| `/ardd-implement` | Execute tasks from `.project/tasks/tasks.md` sequentially |
| `/ardd-converge` | Reconcile codebase with tasks.md after an interrupted `/ardd-implement` run |
