...
# frontmatter is `status: completed`, bound to a plan whose own branch
# (frontmatter `branch:`) has already merged into the default branch, but
# whose bound features are still `Status: tasked` in features.md rather
# than `implemented`. /ardd-implement's and /ardd-converge's post-merge
# flip step assumes a live coordinating conversation checks back after the
# worktree branch merges — but merge is manual/async, so in the common case
# that conversation is gone before it happens and the flip never lands.
...
# tasks file isn't `completed`, its plan's branch isn't merged, or none of
...
branch="$(frontmatter_field "$plan_file" branch)"
[ -n "$branch" ] || exit 0
