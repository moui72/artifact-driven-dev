...
- [x] T003 [parallel] Delete the legacy `## Recent Releases` / `## Feature Backlog` section from `.project/STATUS.md` (currently the file's final ~117 lines, starting at the `## Recent Releases` heading), leaving the `_Updated:` log as the file's sole content below its header. Do not touch any other part of the file — this is a manual one-time removal, not a change to `/ardd-status`'s generation logic.
