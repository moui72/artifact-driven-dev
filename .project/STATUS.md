...
1 open — `feedback-new-sh-codex-undocumented-b8f0.md` (1 bug, filed
today: `docs/install.md` documents `install.sh --harness codex` but
never mentions `new.sh --harness claude|codex`'s identical Codex support
— interactive `ask_harness` prompt, `--harness=<name>` flag, adapted
Codex handoff; a reader would have no way to discover `new.sh` can
target Codex at all). Delivered today: `feedback-badge-style-variant-followups-dbff.md`
...
`/ardd-plan feedback-new-sh-codex-undocumented-b8f0.md` to fix the
`new.sh --harness codex` doc gap in `docs/install.md` (already pushed:
the pre-commit hook fix and docs-sweep scope extension, `be71d6c..0a4df4f`,
land as part of the next beta automatically). Also standing:
...
`hooks/pre-commit`, and the widened `docs-sweep` scope); the two
