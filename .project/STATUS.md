...
- `tasks-new-sh-codex-docs-2770.md` — plan
  `plan-new-sh-codex-docs-2026-07-21-302a.md`, features `none`: no other
  `ready` tasks file and nothing in flight to compare against (sole
  `ready` entry — `parallel-matrix.sh` stays silent below two
  participants).

## In Flight

- Nothing in flight — `worktree-agent-af5346a43bebe9e32`
  (`changelog-from-github-releases` + `pre-commit-hook-scoping-expans`)
  merged (`ff3dabb`) and was reaped 2026-07-21;
  `worktree-agent-a49dbb827b627b722` (`badge-style-variant-option`) merged
  (`a1de0f2`) and was reaped earlier the same day; the codex-port worktree
  was also reaped 2026-07-21 after its branch (`codex/port-foundation`,
  8a302c8) merged to `main`.

## Recommended Next Step

Run `/ardd-implement` on `tasks-new-sh-codex-docs-2770.md` (1 task,
`ready`) to land the `new.sh --harness codex` doc fix in
`docs/install.md`. Two unpushed commits from this session's docs-sweep
work (`937479e`, `66e4b45..937479e`) will publish as part of the next
beta once pushed. Also standing: `/ardd-defects` to refresh the
