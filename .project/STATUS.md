...
2 open. `feedback-new-sh-codex-launch-tty-b951.md` is now `planned`,
consumed by `plan-b860-2026-07-22-e518.md` — see the `_Updated` note
above. `feedback-ardd-backlog-codex-syntax-1c0f.md` (1 bug: every
skill's prose hardcodes Claude's `/ardd-*` invocation syntax even when
installed into Codex's `.agents/skills/`, where the correct sigil is
`$ardd-*` — systemic, not skill-specific — see the `_Updated` note
...
- Nothing in flight — `worktree-agent-a2f2806ef246ff447` (`b860`,
...
`/ardd-plan` to consume the 2 open feedback files (`feedback-ardd-backlog-codex-syntax-1c0f.md`, 1 item —
the systemic `/` vs `$` skill-prose drift, itself flagged as needing a
broader fix pass rather than a single-file patch;
