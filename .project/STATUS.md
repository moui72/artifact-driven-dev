...
**v1.1.0 (2026-07-22)** — stable cut (minor bump) carrying multi-harness
install support (`install.sh --harness codex`), badge/shieldcn styling,
brand refresh, and the accumulated post-v1.0.4 fixes to stable consumers.
Shipped after a clean full-tier `/scenario-sweep` (S1-S8, run
2026-07-21-d816) found no release-blocking issues — see the `_Updated`
note above. **v0.10.2 (2026-07-18)** — stable cut fixing the phantom-completion gap
...
38 implemented (newest: `changelog-from-github-releases`,
`pre-commit-hook-scoping-expans`, `ff3dabb`) ·
1 retired — see
...
`backlogged` → `tasked` → `implemented` this session — see the
`_Updated` note above.
