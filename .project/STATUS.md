...
Push `main` to publish `changelog-from-github-releases`,
`pre-commit-hook-scoping-expans` (this session, `ff3dabb`), and
`badge-style-variant-option` (`a1de0f2`, still unpushed from earlier
today) as part of the next beta. Also standing: `/ardd-defects` to
refresh the 2026-07-12 seventh pass against the enlarged surface (now
including `scripts/release-notes.sh` and the expanded `hooks/pre-commit`);
the two remaining backlogged features (`plan-preview-editor-option`;
