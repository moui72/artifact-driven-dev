...
`plan-preview-editor-option`, `badge-style-variant-option`) ·
...
Push `main` to publish the accumulated batch (codex foundation,
multi-harness metadata, hook P90, brand refresh) as the next beta, then
dispatch `stable-release.yml` when ready. Then: /ardd-plan over
feedback-badge-style-clash-7db2 and/or the three backlogged features
(badge-style-variant-option pairs naturally with 7db2;
plan-preview-editor-option; codex-second-harness-support via --from its
draft plan). v1.0.4 is Latest; consumer repos can /ardd-update. Also
standing:
