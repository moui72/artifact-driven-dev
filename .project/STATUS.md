...
**Manual, not a skill invocation**: live-verify the Codex launch-tty fix
in a real terminal (`codex '$ardd-status'` or similar against a real
Codex-harness project) — no session so far, delegated or not, has had a
controlling tty to confirm it. Then `/ardd-plan` to consume the 2 open
feedback files (`feedback-ardd-backlog-codex-syntax-1c0f.md`, 1 item —
