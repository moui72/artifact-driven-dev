...
0 open. Delivered today: `feedback-new-sh-codex-undocumented-b8f0.md`
...
Push `main` to publish this session's work as part of the next beta —
`be71d6c..4f0177c` is unpushed (the pre-commit hook fix, both
docs-sweep extensions, and the `new.sh` Codex doc fix). Also standing:
`/ardd-defects` to refresh the 2026-07-12 seventh pass against the
enlarged surface (now including `scripts/release-notes.sh`, the expanded
`hooks/pre-commit`, and the twice-widened `docs-sweep` scope); the two
remaining backlogged features (`plan-preview-editor-option`;
